...
```
Implement Notion-driven subscribe form field visibility + copy using the shared GitHub package `subscribe-form-config` (same pattern as Lavessia / brand-template).
...
Client: `import { useSubscribeFormConfig, isSubscribeFieldVisible, subscribeFieldText } from 'subscribe-form-config/client'`
...
- use `subscribeFieldText(config, key, fallback)` for labels / checkbox copy
- validation skips required checks for **hidden** fields
...
## 4. Validation rules (keep; skip if field hidden)
- firstName / lastName required if visible
- Terms required if `cbTerms` visible
...
- `cbMarketing` never required by itself
...
Debug: `GET /api/subscription/form-config` → `meta.usedFallback`, `meta.issues`, `config.*.visible`.
...
Add/select Brand = this site's domain (e.g. `example.com`). Toggle `*_visible`. Empty text cells keep code defaults.

## Done when
- With a valid Notion row, `*_visible` shows/hides fields
- Text columns override labels/checkbox copy
- Broken pairs → full default form (no 400 from missing consent)
- Without Notion env, form behaves as before
```
